...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = User.query.filter_by(email=email).first()
        if not user:
            flash('The email does not match. Please try again')
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    return render_template("post.html", post=requested_post, form=comment, logged_in=current_user.is_authenticated, comments=all_comments)
